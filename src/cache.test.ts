 ...
 *   - it caches parcels up to a byte capacity the operator sets, and never past it;
 *   - over capacity, it evicts the LEAST-RECENTLY-USED cached parcel to make room;
 *   - a PINNED parcel (one a publisher is paying to keep available) is never evicted, even under
...
test('it holds parcels and reads them back, up to the capacity it was given', () => {
  const c = new ParcelCache(1000);
...
test('over capacity, it evicts the least-recently-used cached parcel', () => {
  const c = new ParcelCache(1000);
  ...
  assert.equal(c.has('f', 0), false, 'the oldest cached parcel was dropped');
  assert.equal(c.has('f', 1), true);
  assert.equal(c.has('f', 2), true);
  assert.ok(c.usedBytes() <= 1000);
});

test('reading a parcel makes it recently used, so it survives the next eviction', () => {
  const c = new ParcelCache(1000);
  ...
  assert.equal(c.has('f', 0), true, 'the recently-read parcel stayed');
  assert.equal(c.has('f', 1), false, 'the untouched one was dropped');
});

test('a pinned parcel is never evicted, even when the cache is under pressure', () => {
  const c = new ParcelCache(1000);
  ...
  assert.equal(c.has('f', 0), true, 'the pinned parcel is kept');
  assert.equal(c.has('f', 1), false, 'the evictable one went instead');
  assert.equal(c.has('f', 2), true);
});

test('if only pinned parcels remain and there is no room, the new parcel is refused', () => {
  const c = new ParcelCache(1000);
...
test('a parcel larger than the whole capacity is refused outright', () => {
  const c = new ParcelCache(1000);
