 ...
 *   - a provider that served junk is flagged to slash -- and if it also served good babels, it is
...
const receipt = (over: Partial<Receipt>): Receipt => ({
  perProvider: {}, totalSompi: 0, babelsGot: 0, bytesGot: 0, complete: true, faults: [], ...over,
});

test('each earner with a channel is paid exactly what it earned, on that channel', () => {
  const r = receipt({
    perProvider: { 'http://a': { babels: 3, bytes: 300, sompi: 600 }, 'http://b': { babels: 1, bytes: 100, sompi: 200 } },
    totalSompi: 800,
  });
...
test('an earner with no channel is reported unsettleable, not silently dropped', () => {
  const r = receipt({ perProvider: { 'http://a': { babels: 1, bytes: 100, sompi: 200 } }, totalSompi: 200 });
...
test('a mixed provider is BOTH paid for good babels and slashed for junk', () => {
  const r = receipt({
    perProvider: { 'http://m': { babels: 2, bytes: 200, sompi: 400 } },
