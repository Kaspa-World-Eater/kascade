/**
 * What shoal promises, pinned:
 *   - a file reassembles BYTE-IDENTICAL from several providers pulled at once;
 *   - each provider is paid for exactly the chunks IT served, and the parts sum to the whole price;
 *   - a provider that serves JUNK is caught against the manifest, paid nothing for it, and routed
 *     around -- the file still completes;
 *   - if no honest holder has a chunk, the file is honestly INCOMPLETE, never corrupted;
 *   - stopping partway pays only for the chunks that arrived.
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import type { Server } from 'node:http';
import { buildManifest, verifyChunk, manifestIsSound, type Manifest } from './manifest.js';
...
/** Every chunk's bytes, by index, cut from the source the same way buildManifest cut it. */
function cut(bytes: Uint8Array, m: Manifest): Map<number, Uint8Array> {
  const out = new Map<number, Uint8Array>();
  for (const c of m.chunks) out.set(c.index, bytes.subarray(c.index * m.chunkSize, c.index * m.chunkSize + c.size));
  return out;
}
...
test('a manifest is content-addressed: it is sound, and a tampered chunk fails to verify', () => {
  const bytes = file(200_000);
  const m = buildManifest('data.bin', bytes, 64 * 1024);
  assert.equal(m.chunks.length, 4);
  assert.equal(manifestIsSound(m), true);
  const chunk0 = cut(bytes, m).get(0) as Uint8Array;
  assert.equal(verifyChunk(m, 0, chunk0), true);
  const tampered = Uint8Array.from(chunk0); tampered[0] = (tampered[0] ?? 0) ^ 0xff;
  assert.equal(verifyChunk(m, 0, tampered), false, 'one flipped bit is caught');
});

test('THE SWARM: a file reassembles byte-for-byte from three providers, each paid for its share', async () => {
  const bytes = file(500_000); // 8 chunks at 64 KB
  const m = buildManifest('movie.bin', bytes, 64 * 1024);
  const all = cut(bytes, m);
  // three providers, overlapping subsets whose union is every chunk.
  const subsets = [[0, 1, 2, 3, 4], [3, 4, 5, 6], [0, 6, 7]];
  const held = (idx: number[]): Held[] => [{ manifest: m, chunks: new Map(idx.map((i) => [i, all.get(i) as Uint8Array])) }];
  ...
  try {
    ...
    assert.equal(receipt.chunksGot, 8);
...
test('a JUNK provider is caught and routed around; the file still completes and it earns nothing for junk', async () => {
  const bytes = file(300_000); // 5 chunks
  const m = buildManifest('data.bin', bytes, 64 * 1024);
  const all = cut(bytes, m);
  const honest = provider({ held: [{ manifest: m, chunks: new Map(m.chunks.map((c) => [c.index, all.get(c.index) as Uint8Array])) }], priceSompi: PRICE });
  // a liar that holds every chunk too, but flips a byte in whatever it serves.
  const liar = provider({
    held: [{ manifest: m, chunks: new Map(m.chunks.map((c) => [c.index, all.get(c.index) as Uint8Array])) }],
    priceSompi: PRICE,
  ...
  try {
    const holders: Holder[] = [
      { url: liar.url(), indices: m.chunks.map((c) => c.index) },   // listed first, tried first
      { url: honest.url(), indices: m.chunks.map((c) => c.index) },
    ];
...
test('if no honest holder has a chunk, the file is honestly incomplete, not corrupted', async () => {
  const bytes = file(200_000); // 4 chunks
  const m = buildManifest('data.bin', bytes, 64 * 1024);
  const all = cut(bytes, m);
  // one provider that holds only chunks 0 and 1 -- 2 and 3 exist nowhere.
  const p = provider({ held: [{ manifest: m, chunks: new Map([[0, all.get(0) as Uint8Array], [1, all.get(1) as Uint8Array]]) }], priceSompi: PRICE });
  ...
  try {
    ...
    assert.equal(receipt.complete, false, 'missing chunks means incomplete');
    assert.equal(receipt.chunksGot, 2);
    assert.equal(receipt.totalSompi, receipt.bytesGot * PRICE, 'paid only for the two real chunks');
  } finally {
    await close(p.server);
  }
});

test('stopping partway pays only for the chunks that arrived', async () => {
  const bytes = file(640_000); // 10 chunks
  const m = buildManifest('big.bin', bytes, 64 * 1024);
  const all = cut(bytes, m);
  const p = provider({ held: [{ manifest: m, chunks: new Map(m.chunks.map((c) => [c.index, all.get(c.index) as Uint8Array])) }], priceSompi: PRICE });
  await listen(p.server);
  try {
    let got = 0;
    const holders: Holder[] = [{ url: p.url(), indices: m.chunks.map((c) => c.index) }];
    const { receipt } = await fetchFile({
      manifest: m, holders, priceSompi: PRICE, concurrency: 1,
      onChunk: () => { got += 1; }, stop: () => got >= 3,
    });
    assert.equal(receipt.complete, false);
    assert.ok(receipt.chunksGot >= 3 && receipt.chunksGot < 10, `stopped promptly, at ${receipt.chunksGot}`);
